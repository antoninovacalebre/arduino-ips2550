...
void print_current_ips_config(IPS2550 *a_ips);
...
double estimate_vtx_rms(IPS2550 *a_ips, ADS131M0x *a_adc) {
  ...
  double rx1_n = adc.get_channel_voltage(2);
  ...
  double rx1_p = adc.get_channel_voltage(2);

  Serial.print(" ");
  Serial.println(rx1_p);
...
double estimate_vtx_pp(IPS2550 *a_ips, ADS131M0x *a_adc) {
  return estimate_vtx(a_ips, a_adc) * 2.0;
}

void print_current_ips_config(IPS2550 *a_ips) {
  Serial.println("-----------------------------");
  Serial.print("Supply Voltage           ");
  Serial.println(a_ips->get_vdd() == VDD_3V3 ? "3.3 V" : "5 V");
  Serial.print("Output Mode              ");
  Serial.println(a_ips->get_output_mode() == DIFFERENTIAL ? "Differential" : "Single Ended");
  Serial.print("Automatic Gain Control   ");
  Serial.println(a_ips->get_automatic_gain_control() == 1 ? "True" : "False");
  Serial.print("Master Gain Code         ");
  Serial.println(a_ips->get_master_gain_code());
  Serial.print("Master Gain Boost (2x)   ");
  Serial.println(a_ips->get_master_gain_boost() == 1 ? "True" : "False");
  Serial.print("Fine Gain 1              ");
  Serial.print(a_ips->get_fine_gain_1_code());
  Serial.print(" (");
  Serial.print(a_ips->get_fine_gain_1());
  Serial.println("x)");
  Serial.print("Fine Gain 2              ");
  Serial.print(a_ips->get_fine_gain_2_code());
  Serial.print(" (");
  Serial.print(a_ips->get_fine_gain_2());
  Serial.println("x)");
  Serial.print("Offset 1                 ");
  Serial.print(a_ips->get_offset_sign_1() * a_ips->get_offset_code_1());
  Serial.print(" (");
  Serial.print(a_ips->get_offset_perc_1() * 100.0);
  Serial.println("% Vtx)");
  Serial.print("Offset 2                 ");
  Serial.print(a_ips->get_offset_sign_2() * a_ips->get_offset_code_2());
  Serial.print(" (");
  Serial.print(a_ips->get_offset_perc_2() * 100.0);
  Serial.println("% Vtx)");
  Serial.print("TX Current Bias          ");
  Serial.print(a_ips->get_tx_current_bias_uA());
  Serial.println(" uA");
  Serial.print("TX Frequency             ");
  Serial.print(a_ips->get_tx_frequency() / 1e6);
  Serial.println(" MHz");
  Serial.println("-----------------------------");
}
